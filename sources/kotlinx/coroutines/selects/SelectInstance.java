...
public final class SelectInstance<R> extends LockFreeLinkedListHead implements SelectBuilder<R>, kotlinx.coroutines.selects.SelectInstance<R>, Continuation<R>, CoroutineStackFrame {
    ...
    public final void handleBuilderException(Throwable e) {
        if (trySelect()) {
            ...
            resumeWith(Result.m21constructorimpl(ResultKt.createFailure(e)));
        } else {
    ...
    public void resumeWith(Object result) {
        ...
        while (true) {
            ...
            if (obj4 == obj) {
                ...
                obj3 = SelectKt.RESUMED;
                if (atomicReferenceFieldUpdater2.compareAndSet(this, coroutine_suspended, obj3)) {
                    if (Result.m27isFailureimpl(result)) {
                        Continuation<R> continuation = this.uCont;
                        Throwable m24exceptionOrNullimpl = Result.m24exceptionOrNullimpl(result);
                        Intrinsics.checkNotNull(m24exceptionOrNullimpl);
                        Result.Companion companion = Result.INSTANCE;
                        if (DebugKt.getRECOVER_STACK_TRACES() && (continuation instanceof CoroutineStackFrame)) {
                            m24exceptionOrNullimpl = StackTraceRecoveryKt.recoverFromStackFrame(m24exceptionOrNullimpl, (CoroutineStackFrame) continuation);
                        }
                        continuation.resumeWith(Result.m21constructorimpl(ResultKt.createFailure(m24exceptionOrNullimpl)));
                        return;
                    }
        ...
        while (true) {
            ...
            if (obj4 == obj) {
                ...
                if (atomicReferenceFieldUpdater2.compareAndSet(this, coroutine_suspended, obj3)) {
                    ...
                    Result.Companion companion = Result.INSTANCE;
                    intercepted.resumeWith(Result.m21constructorimpl(ResultKt.createFailure(exception)));
                    return;
                }
            } else {
                throw new IllegalStateException("Already resumed");
